...
const notifItems = notifs.map( (obj, idx)=>{
  if (idx==notifs.length-1)
    return(
      <div className="clt_notif-line-wrapper-final" key={idx}>
        <p className="clt_notif-line-title">{obj.title}</p>
        <p className="clt_notif-line-main">{obj.main}</p>
        <p className="clt_notif-line-time">{obj.time}</p>
      </div>
    );
  else return(
    <div className="clt_notif-line-wrapper" key={idx}>
      <p className="clt_notif-line-title">{obj.title}</p>
      <p className="clt_notif-line-main">{obj.main}</p>
      <p className="clt_notif-line-time">{obj.time}</p>
    </div>
...
const Notification = () => {
  return (
    <div className="clt_notif-wrapper">
      ...
      <span className="clt_announce-head">個人通知</span>
